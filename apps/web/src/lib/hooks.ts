...
export function useProfileCard(userId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      if (DEMO_MODE) return demoDiscover.find((p) => p.userId === userId) ?? null;
...
export function useSavedProfiles() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ProfileCard[]> => {
      if (DEMO_MODE) return demoDiscover.filter((p) => saved[p.userId]);
...
export function useConversation(conversationId: string) {
  ...
  return useQuery({
    queryKey: ['messages', conversationId, DEMO_MODE ? demoMessages.length : 0],
...
export function useSendMessage(conversationId: string) {
  ...
  return useMutation({
    mutationFn: async (body: string) => {
      if (DEMO_MODE) return sendDemo(conversationId, body);
...
export function useReport() {
  return useMutation({
    mutationFn: async (input: {
      ...
      if (DEMO_MODE) return { id: 'demo-report' };
...
export function useBlockUser() {
  ...
  return useMutation({
    mutationFn: async (userId: string) => {
      if (DEMO_MODE) return { blocked: true };
...
export function useDisconnect() {
  ...
  return useMutation({
    mutationFn: async (matchId: string) => {
      if (DEMO_MODE) return { ended: true };
...
export function useJoinGroup() {
  ...
  return useMutation({
    mutationFn: async ({ groupId, message }: { groupId: string; message?: string }) => {
      if (DEMO_MODE) return { joined: true, pending: false };
...
export function useEventDetail(eventId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<EventSummary | null> => {
      if (DEMO_MODE) return demoEvents.find((e) => e.id === eventId) ?? null;
...
export function calendarUrl(eventId: string): string {
  return DEMO_MODE ? '#' : api().events.calendarUrl(eventId);
}
...
export function usePrices() {
  return useQuery({
    queryKey: ['prices'],
    queryFn: async () => {
      if (DEMO_MODE) return DEFAULT_PRICES;
...
export function useNotifications() {
  return useQuery({
    ...
    queryFn: async (): Promise<NotificationItem[]> => {
      if (DEMO_MODE) return demoNotifications;
