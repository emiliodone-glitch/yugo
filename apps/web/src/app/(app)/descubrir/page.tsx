...
export default function DiscoverPage() {
  ...
  const remaining =
    data?.limit === null || data?.limit === undefined ? null : Math.max(0, data.limit - data.used);
  ...
  const handleInterest = async (userId: string) => {
    try {
      ...
      if (
        message === 'daily_interests_used' ||
        errorMessage(error) === es.errors.dailyInterestsUsed
      ) {
        router.push('/plus');
        return;
      }
